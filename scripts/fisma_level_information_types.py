# This script updates the fisma_level app's fisma_level module,
# which walks the user through choosing information types and
# then confirming the high water mark level for all of the selected
# types, across confidentiality, integrity, and availability.
...
# Create the root question that kicks off which information categories and types
# the user is asked about.
infotype_questions["information_types_categories"] = OrderedDict([
	("id", "information_types_categories"),
	("title", "Information Types Categories"),
	("prompt", "Which categories of information does your system contain?\n\nCheck all that apply.\n"),
	("type", "multiple-choice"),
	("choices", []),
	])

# Recursively process the raw information types data tracing a path to
# the actual information types (i.e. leaf nodes in the tree), through the
	...
	for subtype in infotypes:
		if "subtypes" not in subtype:
			...
			infotype_questions.setdefault(p_key,
				OrderedDict([
					("id", p_key),
					("title", parent_q["title"] + " Information Types"),
					("prompt", "Which types of %s information does your system contain?\n\nCheck all that apply.\n" % parent_q["title"]),
					("type", "multiple-choice"),
					("choices", []),
					("impute", [
						OrderedDict([
					...
					OrderedDict([
						("key", subtype["code"]),
						("text", subtype["title"]),
						("help", subtype["description"]["text"].strip() + " (NIST SP 800-60 Vol II Rev 1 " + subtype["code"] + ")" + ("\n" if "\n" in subtype["description"]["text"].strip() else "")), # YAML is prettier if multi-line strings end with a newline
					])			
			)
...
# Add a final always-imputed question that collects all of the infotypes selected.
...
infotype_questions["information_types"] = OrderedDict([
	...
	("title", "Information types contained in the system"),
...
for impact_type in ("confidentiality", "integrity", "availability"):
	...
	infotype_questions["information_types_%s_provisional" % impact_type] = OrderedDict([
		...
		("title", "Provisional %s impact level" % impact_type),
		("prompt", "[not used, always imputed]"),
		("type", "text"),
		("impute",
			[OrderedDict([
...
module = rtyaml.load(open("../apps/fisma_level/fisma_level.yaml"))
...
# information types of) the provisional categorization given by NIST SP 800-60 Vol II.
...
with open("../apps/fisma_level/fisma_level.yaml", "w") as f:
